...
class DetectionService extends ChangeNotifier {
  ...
  final FaceDetector _faceDetector = FaceDetector(
    options: FaceDetectorOptions(
      ...
      performanceMode:
          FaceDetectorMode.accurate, // Changed to accurate for better detection
  ...
  final List<Rect> _facePositionHistory = [];
  ...
  Future<void> _loadSettings() async {
    ...
    isDrowsinessDetectionEnabled =
        prefs.getBool(PreferenceKeys.isDrowsinessDetectionEnabled) ?? true;
    isDistractionDetectionEnabled =
        prefs.getBool(PreferenceKeys.isDistractionDetectionEnabled) ?? true;
    isSeatbeltDetectionEnabled =
        prefs.getBool(PreferenceKeys.isSeatbeltDetectionEnabled) ?? true;
    isYawningDetectionEnabled =
        prefs.getBool(PreferenceKeys.isYawningDetectionEnabled) ?? true;
    isPassengerMonitoringEnabled =
        prefs.getBool(PreferenceKeys.isPassengerMonitoringEnabled) ?? true;
    isMotionSicknessDetectionEnabled =
        prefs.getBool(PreferenceKeys.isMotionSicknessDetectionEnabled) ??
            true; // Enable by default
    isPhoneUsageDetectionEnabled =
        prefs.getBool(PreferenceKeys.isPhoneUsageDetectionEnabled) ??
            true; // Enable by default
    isSmokingDetectionEnabled =
        prefs.getBool(PreferenceKeys.isSmokingDetectionEnabled) ?? false;
    isErraticMovementDetectionEnabled =
        prefs.getBool(PreferenceKeys.isErraticMovementDetectionEnabled) ?? true;
    isVoiceAlertsEnabled =
        prefs.getBool(PreferenceKeys.isVoiceAlertsEnabled) ?? true;
  ...
  Future<void> saveSettings() async {
    ...
    await prefs.setBool(PreferenceKeys.isDrowsinessDetectionEnabled,
        isDrowsinessDetectionEnabled);
    await prefs.setBool(PreferenceKeys.isDistractionDetectionEnabled,
        isDistractionDetectionEnabled);
    await prefs.setBool(
        PreferenceKeys.isSeatbeltDetectionEnabled, isSeatbeltDetectionEnabled);
    await prefs.setBool(
        PreferenceKeys.isYawningDetectionEnabled, isYawningDetectionEnabled);
    await prefs.setBool(PreferenceKeys.isPassengerMonitoringEnabled,
        isPassengerMonitoringEnabled);
    await prefs.setBool(PreferenceKeys.isMotionSicknessDetectionEnabled,
        isMotionSicknessDetectionEnabled);
    await prefs.setBool(PreferenceKeys.isPhoneUsageDetectionEnabled,
        isPhoneUsageDetectionEnabled);
    await prefs.setBool(
        PreferenceKeys.isSmokingDetectionEnabled, isSmokingDetectionEnabled);
    await prefs.setBool(PreferenceKeys.isErraticMovementDetectionEnabled,
        isErraticMovementDetectionEnabled);
    await prefs.setBool(
        PreferenceKeys.isVoiceAlertsEnabled, isVoiceAlertsEnabled);
  }
  ...
  Future<void> processImage(InputImage inputImage) async {
    ...
    if (_lastImageProcessTime != null &&
        now.difference(_lastImageProcessTime!).inMilliseconds <
            AppConstants.imageProcessingDelay) {
    ...
    try {
      ...
      if (faces.isEmpty) {
        ...
        if (DateTime.now().millisecondsSinceEpoch % 10000 < 100) {
          debugPrint(
              '⚠️  No faces detected - check camera position, lighting, or face detector settings');
          debugPrint(
              '   📊 Image info: ${inputImage.metadata?.size.width.toInt() ?? 'unknown'}x${inputImage.metadata?.size.height.toInt() ?? 'unknown'}');
          debugPrint(
              '   🔧 Face detector settings: minFaceSize=${_faceDetector.options.minFaceSize}, mode=${_faceDetector.options.performanceMode}, classification=${_faceDetector.options.enableClassification}');
        }
      ...
      if (DateTime.now().millisecondsSinceEpoch % 5000 < 100) {
        ...
        debugPrint(
            '   📊 Image info: ${inputImage.metadata?.size.width.toInt()}x${inputImage.metadata?.size.height.toInt()}');
        debugPrint(
            '   🔧 Face detector settings: minFaceSize=${_faceDetector.options.minFaceSize}, mode=${_faceDetector.options.performanceMode}, classification=${_faceDetector.options.enableClassification}');
      }
      ...
      if (isDrowsinessDetectionEnabled) {
        ...
        if (DateTime.now().millisecondsSinceEpoch % 5000 < 100) {
          debugPrint(
              '👁️ Eyes: L=${leftEyeOpen.toStringAsFixed(2)}, R=${rightEyeOpen.toStringAsFixed(2)}, Avg=${avgEyeOpen.toStringAsFixed(2)}');
        }
        ...
        if (avgEyeOpen < AppConstants.eyeClosureThreshold) {
          ...
          debugPrint(
              '😴 Eyes closed detected! Frame $consecutiveDrowsyFrames/${AppConstants.drowsinessFrameThreshold}');

          if (consecutiveDrowsyFrames >=
              AppConstants.drowsinessFrameThreshold) {
            final now = DateTime.now();
            debugPrint(
                '😴 Drowsiness threshold reached! Last alert: $lastDrowsinessAlert');

            if (lastDrowsinessAlert == null ||
                now.difference(lastDrowsinessAlert!).inMilliseconds >
                    AppConstants.drowsinessAlertCooldown) {
              debugPrint(
                  '� DROWSINESS ALERT TRIGGERED! Consecutive frames: $consecutiveDrowsyFrames');
              ...
              final cooldownRemaining = AppConstants.drowsinessAlertCooldown -
                  now.difference(lastDrowsinessAlert!).inMilliseconds;
              debugPrint(
                  '😴 Drowsiness detected but in cooldown (${cooldownRemaining}ms remaining)');
            }
          }
        } else {
          if (consecutiveDrowsyFrames > 0) {
            debugPrint(
                '👁️ Eyes opened - resetting drowsy frame count from $consecutiveDrowsyFrames to 0');
          }
          consecutiveDrowsyFrames = 0;
        }
      }
      ...
      if (isDistractionDetectionEnabled) {
        ...
        if (face.headEulerAngleY != null) {
          ...
          if (DateTime.now().millisecondsSinceEpoch % 4000 < 100) {
            debugPrint(
                '🔄 Distraction check - Head yaw: ${yaw.toStringAsFixed(1)}°, Threshold: ${AppConstants.headPoseThreshold}°');
          }

          if (yaw > AppConstants.headPoseThreshold) {
            isDistracted = true;
            distractionConfidence =
                (yaw / 45.0).clamp(0.0, 1.0); // Normalize to 0-1
          }
        }
        ...
        if (_facePositionHistory.length >= 5) {
          ...
          const movementThreshold = 50.0; // pixels

          // Reduce debug logging frequency for performance
          if (DateTime.now().millisecondsSinceEpoch % 5000 < 100) {
            debugPrint(
                '🔄 Face movement: ${movement.toStringAsFixed(1)}px, Threshold: ${movementThreshold}px');
          }

          if (movement > movementThreshold) {
            isDistracted = true;
            distractionConfidence =
                (movement / 100.0).clamp(0.0, 1.0); // Normalize to 0-1
          }
        }
        ...
        if (isDistracted) {
          ...
          if (consecutiveDistractionFrames >=
              AppConstants.distractionFrameThreshold) {
            final now = DateTime.now();
            debugPrint(
                '🔄 Distraction threshold reached! Last alert: $lastDistractionAlert');

            if (lastDistractionAlert == null ||
                now.difference(lastDistractionAlert!).inMilliseconds >
                    AppConstants.distractionAlertCooldown) {
              debugPrint(
                  '🔄 DISTRACTION ALERT TRIGGERED! Consecutive frames: $consecutiveDistractionFrames');
              ...
              final cooldownRemaining = AppConstants.distractionAlertCooldown -
                  now.difference(lastDistractionAlert!).inMilliseconds;
              debugPrint(
                  '🔄 Distraction detected but in cooldown (${cooldownRemaining}ms remaining)');
            }
          }
        } else {
          consecutiveDistractionFrames = 0;
      ...
      if (isYawningDetectionEnabled) {
        ...
        final mouthOpen = face.smilingProbability != null
            ? (1.0 - face.smilingProbability!)
            : 0.0;

        // Reduce debug logging frequency for performance
        if (DateTime.now().millisecondsSinceEpoch % 2000 < 100) {
          debugPrint(
              '🥱 Yawning check - Mouth openness: ${mouthOpen.toStringAsFixed(2)}, Smiling prob: ${face.smilingProbability?.toStringAsFixed(2) ?? "null"}, Threshold: ${AppConstants.mouthAspectRatioThreshold}');
        }

        if (mouthOpen > AppConstants.mouthAspectRatioThreshold) {
          final now = DateTime.now();
          if (lastYawningAlert == null ||
              now.difference(lastYawningAlert!).inMilliseconds >
                  AppConstants.yawningAlertCooldown) {
            debugPrint(
                '🥱 YAWNING ALERT TRIGGERED! Mouth openness: ${mouthOpen.toStringAsFixed(2)}');
            ...
            final cooldownRemaining = AppConstants.yawningAlertCooldown -
                now.difference(lastYawningAlert!).inMilliseconds;
            debugPrint(
                '🥱 Yawning detected but in cooldown (${cooldownRemaining}ms remaining)');
          }
        }
      }
      ...
      if (isMotionSicknessDetectionEnabled) {
        if (face.headEulerAngleX != null && face.headEulerAngleY != null) {
          final totalHeadMovement =
              face.headEulerAngleX!.abs() + face.headEulerAngleY!.abs();

          // Reduce debug logging frequency for performance
          if (DateTime.now().millisecondsSinceEpoch % 3000 < 100) {
            debugPrint(
                '🤢 Motion sickness check - X angle: ${face.headEulerAngleX!.toStringAsFixed(1)}°, Y angle: ${face.headEulerAngleY!.toStringAsFixed(1)}°, Total: ${totalHeadMovement.toStringAsFixed(1)}°, Threshold: ${AppConstants.motionSicknessThreshold * 10}°');
          }
          ...
          if (totalHeadMovement > AppConstants.motionSicknessThreshold * 10) {
            ...
            if ((lastMotionSicknessAlert == null ||
                    now.difference(lastMotionSicknessAlert!).inMilliseconds >
                        AppConstants.motionSicknessAlertCooldown) &&
                (lastYawningAlert == null ||
                    now.difference(lastYawningAlert!).inMilliseconds > 2000)) {
              // 2 second gap from yawning

              debugPrint(
                  '🤢 MOTION SICKNESS ALERT TRIGGERED! Total movement: ${totalHeadMovement.toStringAsFixed(1)}°');
              ...
              final cooldownRemaining =
                  AppConstants.motionSicknessAlertCooldown -
                      now.difference(lastMotionSicknessAlert!).inMilliseconds;
              debugPrint(
                  '🤢 Motion sickness detected but in cooldown (${cooldownRemaining}ms remaining)');
            }
          }
        }
      }
      ...
      if (isSeatbeltDetectionEnabled) {
        ...
        if (faceY > 300 || faceArea > 50000) {
          // Adjusted thresholds
          final now = DateTime.now();
          if (lastSeatbeltAlert == null ||
              now.difference(lastSeatbeltAlert!).inMilliseconds > 30000) {
            // 30 second cooldown

            debugPrint(
                '🔒 SEATBELT ALERT TRIGGERED! Face position: Y=${faceY.toStringAsFixed(1)}, Area=${faceArea.toStringAsFixed(1)}');
            ...
            final cooldownRemaining =
                30000 - now.difference(lastSeatbeltAlert!).inMilliseconds;
            debugPrint(
                '🔒 Seatbelt issue detected but in cooldown (${cooldownRemaining}ms remaining)');
          }
        }
      }
        ...
        if (face.headEulerAngleX != null && face.headEulerAngleY != null) {
          ...
          if (headTilt > 20 && headTurn > 10 && headTilt < 60) {
            ...
            if (lastPhoneUsageAlert == null ||
                now.difference(lastPhoneUsageAlert!).inMilliseconds > 15000) {
              // 15 second cooldown

              debugPrint(
                  '📱 PHONE USAGE ALERT TRIGGERED! Head tilt: ${headTilt.toStringAsFixed(1)}°, turn: ${headTurn.toStringAsFixed(1)}°');
              ...
              final cooldownRemaining =
                  15000 - now.difference(lastPhoneUsageAlert!).inMilliseconds;
              debugPrint(
                  '📱 Phone usage detected but in cooldown (${cooldownRemaining}ms remaining)');
            }
          }
        }
      }
      ...
      if (isErraticMovementDetectionEnabled) {
        if (face.headEulerAngleX != null && face.headEulerAngleY != null) {
          final totalMovement = face.headEulerAngleX!.abs() +
              face.headEulerAngleY!.abs() +
              (face.headEulerAngleZ?.abs() ?? 0);

          // Log occasionally for debugging
          if (DateTime.now().millisecondsSinceEpoch % 4000 < 100) {
            debugPrint(
                'Erratic movement check - Total movement: ${totalMovement.toStringAsFixed(1)}°');
          }

          if (totalMovement > 80.0) {
            final now = DateTime.now();
            if (lastErraticMovementAlert == null ||
                now.difference(lastErraticMovementAlert!).inMilliseconds >
                    3000) {
              debugPrint(
                  '🚨 ERRATIC MOVEMENT ALERT TRIGGERED! Total movement: ${totalMovement.toStringAsFixed(1)}°');
              ...
              final cooldownRemaining = 3000 -
                  now.difference(lastErraticMovementAlert!).inMilliseconds;
              debugPrint(
                  '🚨 Erratic movement detected but in cooldown (${cooldownRemaining}ms remaining)');
            }
          }
        }
      }
        ...
        if (face.headEulerAngleX != null && face.smilingProbability != null) {
          ...
          if (headTilt < -5 && mouthPattern < 0.3) {
            ...
            if (lastSmokingAlert == null ||
                now.difference(lastSmokingAlert!).inMilliseconds > 20000) {
              // 20 second cooldown

              debugPrint(
                  '🚭 SMOKING ALERT TRIGGERED! Head tilt: ${headTilt.toStringAsFixed(1)}°, mouth: ${mouthPattern.toStringAsFixed(2)}');
              _showAlert(
                  "Smoking detected! Please avoid smoking while driving!");

              final detection = DetectionResult(
                type: DetectionType.smokingDetection,
                confidence:
                    ((-headTilt + (1.0 - mouthPattern)) / 10.0).clamp(0.0, 1.0),
                timestamp: now,
              );

              _detectionHistory.addDetection(detection);
              _detectionStats
                  .incrementDetection(DetectionType.smokingDetection);
              lastSmokingAlert = now;
            } else {
              final cooldownRemaining =
                  20000 - now.difference(lastSmokingAlert!).inMilliseconds;
              debugPrint(
                  '🚭 Smoking detected but in cooldown (${cooldownRemaining}ms remaining)');
            }
          }
        }
      }
      ...
      if (isPassengerMonitoringEnabled && faces.length > 1) {
        ...
        final passengerAlertTime = lastPhoneUsageAlert ??
            DateTime(2000); // Use phone usage timestamp as proxy
        if (now.difference(passengerAlertTime).inMilliseconds > 25000) {
          // 25 second cooldown

          debugPrint(
              '👥 PASSENGER DISTURBANCE ALERT TRIGGERED! Face count: ${faces.length}');
          _showAlert(
              "Passenger disturbance detected! Multiple people in driver area!");
          ...
          _detectionHistory.addDetection(detection);
          _detectionStats
              .incrementDetection(DetectionType.passengerDisturbance);
          lastPhoneUsageAlert =
              now; // Reusing this timestamp for passenger alerts
        }
      }
    } catch (e) {
  ...
  void _showAlert(String message) {
    ...
    if (currentAlert == null ||
        _isHigherPriorityAlert(message, currentAlert!)) {
      ...
      Timer(const Duration(milliseconds: AppConstants.alertDisplayDuration), () {
      ...
      debugPrint(
          "🔕 Visual alert skipped - another alert is being displayed: $currentAlert");
    }

    // Always trigger vibration (can happen multiple times)
    Timer(const Duration(milliseconds: AppConstants.vibrationDelay), () {
      _triggerVibration();
    });

    // Always trigger voice alert (with queueing logic in the voice method)
    Timer(const Duration(milliseconds: AppConstants.voiceAlertDelay), () {
  ...
  Future<void> _triggerVoiceAlert(String message) async {
    debugPrint(
        "🔊 Voice alert check - Enabled: $isVoiceAlertsEnabled, Speaking: $_isSpeaking, InProgress: $_isVoiceAlertInProgress");
  ...
  void _startWebDemoMode() {
    ...
    _webDemoTimer = Timer.periodic(const Duration(seconds: 8), (timer) {
      ...
      final randomType =
          detectionTypes[_webDemoCounter % detectionTypes.length];
      final detection = DetectionResult(
        type: randomType,
  ...
  void startDetection() {
    ...
    notifyListeners();
    debugPrint(
        '✅ Detection service started - Running: $isRunning, All cooldowns reset');
  }
