...
class DetectionService extends ChangeNotifier {
  ...
  Future<void> _loadSettings() async {
    ...
    isMotionSicknessDetectionEnabled =
        prefs.getBool(PreferenceKeys.isMotionSicknessDetectionEnabled) ?? false;
    isPhoneUsageDetectionEnabled =
        prefs.getBool(PreferenceKeys.isPhoneUsageDetectionEnabled) ?? false;
    ...
    try {
      ...
      if (DateTime.now().millisecondsSinceEpoch % 1000 < 100) {
        ...
        if (faces.isNotEmpty) {
          ...
          debugPrint(
              'Head angles - X: ${face.headEulerAngleX}, Y: ${face.headEulerAngleY}');
        }
      }
      ...
      if (isDrowsinessDetectionEnabled) {
        ...
        debugPrint(
            'Drowsiness check - Eye openness: ${avgEyeOpen.toStringAsFixed(2)}, Threshold: ${AppConstants.eyeClosureThreshold}');

        if (avgEyeOpen < AppConstants.eyeClosureThreshold) {
          consecutiveDrowsyFrames++;
          debugPrint(
              'Drowsy frame: $consecutiveDrowsyFrames/${AppConstants.drowsinessFrameThreshold}');
      ...
      if (isDistractionDetectionEnabled) {
        if (face.headEulerAngleY != null) {
          ...
          if (yaw > AppConstants.headPoseThreshold) {
            ...
            debugPrint(
                'Distraction frame: $consecutiveDistractionFrames/${AppConstants.distractionFrameThreshold}');
  ...
  Future<void> _triggerVoiceAlert(String message) async {
    debugPrint(
        "Voice alert check - Enabled: $isVoiceAlertsEnabled, Speaking: $_isSpeaking");

    if (!isVoiceAlertsEnabled || _isSpeaking) {
      debugPrint(
          "Voice alert skipped - Enabled: $isVoiceAlertsEnabled, Speaking: $_isSpeaking");
      return;
    }
