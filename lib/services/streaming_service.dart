    ...
    for (int ty = 0; ty < targetHeight; ty++) {
      for (int tx = 0; tx < targetWidth; tx++) {
        ...
        if (yIndex < yPlane.length && uIndex < uPlane.length && vIndex < vPlane.length) {
          ...
          final yScaled = (yValue - 16.0) * 1.164;
          final uScaled = uValue - 128.0;
          final vScaled = vValue - 128.0;
          
          final r = (yScaled + 1.596 * vScaled).round().clamp(0, 255);
          final g = (yScaled - 0.391 * uScaled - 0.813 * vScaled).round().clamp(0, 255);
          final b = (yScaled + 2.018 * uScaled).round().clamp(0, 255);
