...
class _DetectionScreenState extends State<DetectionScreen> {
  ...
  void _startAutomaticDetection() async {
    final detectionService =
        Provider.of<DetectionService>(context, listen: false);
        ...
        child: Column(
          ...
          children: [
            ...
            if (history.isEmpty)
              ...
              ...history
                  .take(5)
                  .map((detection) => _buildHistoryItem(detection, service)),
          ],
        ),
      ),
    );
        ...
        Row(
          children: [
            ...
            Expanded(
              child: ElevatedButton.icon(
                onPressed: () =>
                    service.triggerDemoAlert('passenger_disturbance'),
                style: ElevatedButton.styleFrom(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _buildDetectionTypeRow(
                Icons.visibility_off, 'Drowsiness Detection', Colors.orange),
            _buildDetectionTypeRow(
                Icons.face, 'Distraction Monitoring', Colors.purple),
            _buildDetectionTypeRow(Icons.sentiment_very_dissatisfied,
                'Yawning Detection', Colors.amber),
            _buildDetectionTypeRow(
                Icons.sick, 'Motion Sickness Detection', Colors.teal),
            _buildDetectionTypeRow(
                Icons.warning, 'Erratic Movement Detection', Colors.deepOrange),
            const SizedBox(height: 8),
            Container(
  ...
  Widget _buildHistoryItem(
      DetectionResult detection, DetectionService service) {
    final timeStr =
        '${detection.timestamp.hour.toString().padLeft(2, '0')}:${detection.timestamp.minute.toString().padLeft(2, '0')}';
  ...
  Widget _buildDialogStatistics(DetectionService service) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        _buildStatRow(
            'Drowsiness', stats.drowsinessDetections, Icons.visibility_off),
        _buildStatRow('Distraction', stats.distractionDetections, Icons.face),
        _buildStatRow('Yawning', stats.yawningDetections,
            Icons.sentiment_very_dissatisfied),
        _buildStatRow(
            'Motion Sickness', stats.motionSicknessDetections, Icons.sick),
        _buildStatRow('Phone Usage', stats.phoneUsageDetections, Icons.phone),
        _buildStatRow('No Seatbelt', stats.noSeatbeltDetections,
            Icons.airline_seat_recline_extra),
        _buildStatRow(
            'Erratic Movement', stats.erraticMovementDetections, Icons.warning),
        _buildStatRow('Passenger Disturbance',
            stats.passengerDisturbanceDetections, Icons.people),
        _buildStatRow(
            'Smoking Detection', stats.smokingDetections, Icons.smoking_rooms),
        const SizedBox(height: 12),
        Container(
